...
begin
  ...
  begin
    FFileStream := TBufferedFileStream.Create(dlgOpen2.FileName,
      fmOpenRead or fmShareDenyWrite, 0);
    TextStream := TMemoryStream.Create;
    MemStream := TMemoryStream.Create;
    try
        MemStream.CopyFrom(FFileStream);
        MemStream.Position := 0;
        ObjectBinaryToText(MemStream, TextStream);

       FileStream := TFileStream.Create(ChangeFileExt(dlgOpen2.FileName, '.gpt'), fmCreate);
      try
        TextStream.Position := 0;
        FileStream.CopyFrom(TextStream, TextStream.Size);
      finally
