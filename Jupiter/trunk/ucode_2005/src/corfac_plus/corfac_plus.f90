...
USE UTLUCODE
  ...
  !     MODIFIED TO ACCOMODATE JUPITER API DATA EXCHANGE FILES & UCODE2005
  ...
  PARAMETER (VERSION = '1.007') ! 06/17/2010
  !**FORMAT LIST
  480 FORMAT (34X,'CORFAC_PLUS',//, &
      24X,'CONSTRUCTED USING THE JUPITER API',/, &
      17X,'UCODE_2005 POST-PROCESSING PROGRAM TO CALCULATE',/, &
  ...
  485 FORMAT (//, &
   ...
   5X,'DATA_EXCHANGE FILES GENERATED BY UCODE_2005 WHEN IT IS EXECUTED',/, &
  ...
  1020 FORMAT(1X,'CORFAC_PLUS requires a *.corfac file,',/, &
                 '  refer to the UCODE_2005 User''s Manual for details')
...
!===============================================================================
  ...
  ELSE
    ...
    CLOSE(IOUT)
    CALL UTL_STOP &
    ('CORFAC_PLUS requires file *.corfac, see the UCODE_2005 Manual')
  ENDIF
  ! READ ._DM FILE
...
!===============================================================================
  ...
  !------------------------------------------------------------------
    ...
    100 FORMAT(1X,A)
    480 FORMAT (34X,'CORFAC_PLUS',//, &
        18X,'UCODE_2005 POST-PROCESSING PROGRAM TO CALCULATE',/, &
        1X,'CORRECTION FACTORS FOR CONFIDENCE REGION & CONFIDE', &
           'NCE AND PREDICTION INTERVALS',/, &
        29X,'Version ',A/)
    2000 FORMAT(//, &
