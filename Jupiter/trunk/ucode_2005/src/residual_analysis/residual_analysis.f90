...
USE UTLUCODE
  ...
  !
  PARAMETER (VERSION='1.007') !  08/05/2010
  ! Formats
  ...
  1160 FORMAT (31X,'RESIDUAL_ANALYSIS',//, &
       ...
       23X,'UCODE_2005 POST-PROCESSING PROGRAM',/, &
  ...
  CALL UTLUCODE_DX_READ_DM(IFAIL,OUTNAM, &
                           ...
                           CDUM,CDUM,CDUM,CDUM, &
                           MPR,IDUM,NORIG,NP,NPS,NOBS, &
                           RDUM,RDUM,RDUM,VAR,IOUT,RDUM,RDUM)
  IFAIL = 0
  IF(PR_RD) THEN
  ...
  CALL UTL_DX_READ_MCMV('_mv',NP,OUTNAM,PARNAM,COV)
  IF(PRCOVPARAM) THEN
    WRITE(IOUT,1060)
  ...
  CALL SEN_UEV_DX_READ_SU &
       (NOBS,NPE,OBSNAM,OUTNAM,PANAMNPE,PLOTSYMBOL,XORIG,'_su')
  IF(MPR > 0) &
  CALL SEN_UEV_DX_READ_SU(MPR,NPE,PRINAM,OUTNAM,PANAMNPE, &
                           PLOTSYMBOLPRI,XPRI,'_supri')
  DO I=1,NOBS
    DID(I) = OBSNAM(I)
    ISYM(I) = PLOTSYMBOL(I)
  ENDDO
  ...
  999 WRITE(*,1380)
  WRITE(*,1381) TRIM(TRIM(OUTNAM)//'.#rs')
  IF(PR_RD)WRITE(*,1381)TRIM(TRIM(OUTNAM)//'._rd')
  IF(PR_RG)WRITE(*,1381)TRIM(TRIM(OUTNAM)//'._rg')
  IF(PR_RC)WRITE(*,1381)TRIM(TRIM(OUTNAM)//'._rc')
  IF(PR_RB)WRITE(*,1381)TRIM(TRIM(OUTNAM)//'._rb')
