...
//    DistanceForDrawdown100: double;
    DrawdownStressPeriod1: double;
    DrawdownStressPeriod2: double;
  end;
  ...
  private
    ...
    function PrepareModelMuseFile: boolean;
...
begin
  ...
  try
    ...
    FileIndexes.ParamEndTimePosition1 := PositionIndex + 30;
...
//        SetLength(FResults.Drawdowns, HydModResults.TimeCount);
//        SetLength(FResults.DrawdownTimes, HydModResults.TimeCount);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              ATime := HydModResults.Times[TimeIndex];
//              FResults.Drawdowns[TimeIndex] := AValue;
//              FResults.DrawdownTimes[TimeIndex] := HydModResults.Times[TimeIndex];
              ...
              if Abs((3-ATime)/3) < 1e-7 then
              begin
                FResults.DrawdownStressPeriod2 := AValue;
              end;
...
procedure TRunModelThread.ResetResults;
begin
  FResults.MaxCumulativePercentDiscrepancy := 99999.99;
  FResults.MaxTimeStepPercentDiscrepancy := 99999.99;
  SetLength(FResults.FixedDistanceDrawDowns, 0);
  FResults.MaxDrawdown := 99999.99;
  FResults.DrawdownStressPeriod1 := 99999.99;
  FResults.DrawdownStressPeriod2 := 99999.99;
//  SetLength(FResults.Drawdowns, 0);
//  SetLength(FResults.DrawdownTimes, 0);
end;
...
function TRunModelThread.PrepareModelMuseFile: boolean;
var
  YOffset: Extended;
  XOffset: Extended;
  CalculatedArea: Extended;
...
//    Result := FResults.StressPeriodLength <= 3;
    ...
    FModelMuseFile[FIndicies.ParamEndTimePosition2] := '              EndTime = 3';// + FloatToStr(2*FResults.StressPeriodLength);
    ...
    if FResults.StressPeriodLength > 3 then
    begin
      FModelMuseFile[FIndicies.EndtimePosition2] := '      EndTime = ' + FloatToStr(FResults.StressPeriodLength+1);
      FModelMuseFile[FIndicies.PeriodLengthPosition2] := '      PeriodLength = 1'// + FloatToStr(3-FResults.StressPeriodLength);
    end
    else
    begin
      FModelMuseFile[FIndicies.EndtimePosition2] := '      EndTime = 3';// + FloatToStr(FResults.StressPeriodLength*2);
      FModelMuseFile[FIndicies.PeriodLengthPosition2] := '      PeriodLength = ' + FloatToStr(3-FResults.StressPeriodLength);
    end;
