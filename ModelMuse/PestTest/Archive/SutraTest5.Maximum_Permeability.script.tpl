...
read_list_file(reference_clist='cl_Discretization',skiplines=1, &
  ...
  file='SutraTest5.Maximum_Permeability.PstValues')
read_list_file(reference_clist='cl_Discretization',skiplines=1, &
...
K = @                        K@
# Pilot points are not used with K.
...
# Setting values for layer     1
  # Setting values for parameter K
    ...
    p_Value1(select=(s_PIndex1 == 1)) = p_Value1 * K
temp2=new_plist(reference_clist=cl_Discretization,value=0.0)
# Setting values for layer     2
  # Setting values for parameter K
    # Substituting parameter values in zones
    p_Value2(select=(s_PIndex2 == 1)) = p_Value2 * K
temp3=new_plist(reference_clist=cl_Discretization,value=0.0)
# Setting values for layer     3
  # Setting values for parameter K
    # Substituting parameter values in zones
    p_Value3(select=(s_PIndex3 == 1)) = p_Value3 * K

#Write new data values
write_column_data_file(header='no', &
  file='arrays\SutraTest5.Maximum_Permeability_1.arrays';delim="space", &
  plist=p_Value1)
write_column_data_file(header='no', &
  file='arrays\SutraTest5.Maximum_Permeability_2.arrays';delim="space", &
  plist=p_Value2)
write_column_data_file(header='no', &
  file='arrays\SutraTest5.Maximum_Permeability_3.arrays';delim="space", &
  plist=p_Value3)
