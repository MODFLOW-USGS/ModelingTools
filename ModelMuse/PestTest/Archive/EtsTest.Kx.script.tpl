...
cl_Discretization = read_mf_grid_specs(file="C:\ModelingTools\ModelMuse\PestTest\EtsTest.gsf")#Read data to modify
read_list_file(reference_clist='cl_Discretization',skiplines=1, &
  slist=s_PIndex1;column=2, &
  plist=p_Value1;column=3, &
  file='EtsTest.Kx.PstValues')
read_list_file(reference_clist='cl_Discretization',skiplines=1, &
...
write_column_data_file(header='no', &
  file='arrays\EtsTest.Kx_1.arrays';delim="space", &
  plist=p_Value1)
write_column_data_file(header='no', &
  file='arrays\EtsTest.Kx_2.arrays';delim="space", &
  plist=p_Value2)
write_column_data_file(header='no', &
  file='arrays\EtsTest.Kx_3.arrays';delim="space", &
  plist=p_Value3)
