...
type
  TFormPriorInfo = class(TForm)
    ...
    procedure rbwdgPriMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
  private
    ...
    begin
      for IGp := 0 to NGps - 1 do
        begin
          ...
          PriGpsLocal.Items[IGp].Name := ConvertString20(TempString);
        ...
        begin
          ...
          if J_Valid_Name(TempString,MaxLenDepName) then
            begin
              ...
              PriSetLocal.Items[IPri].Name := ConvertString20(TempString);
          ...
          if (RowContainsSelectedCell(rbwdgPri,I))
            ...
            begin
              if rbwdgPri.RowCount > 2 then
                ...
                begin
                  rbwdgPri.Cells[1,1] := '';
                  for J := 2 to rbwdgPri.ColCount-1 do
                    begin
                      rbwdgPri.Cells[J,1] := '';
                    end;
              ...
              DelRow := I;
              // Delete corresponding item in PriSetLocal.
              if (PriSetLocal.Count > 0) then PriSetLocal.Delete(I-1);
            end;
        end;
      if DelRow > 0 then
        begin
                ...
                begin
                  ...
                  if K = 0 then
                    begin
                      ...
                      if PriGpsLocal.Count > 0 then PriGpsLocal.Delete(I-1);
                    end
                  else
                    begin
...
begin
  for IRowCurr := 1 to rbwdgPriGps.RowCount - 1 do
    begin
      OldGroup := ConvertString(PriGpsLocal.Items[IRowCurr - 1].Name);
...
begin
  ...
  begin
    IP1 := IGP + 1;
    rbwdgPriGps.Cells[0,IP1] := ConvertString(psSource.Items[IGP].Name);
    J := 0;
...
procedure TFormPriorInfo.rbwdgPriMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  NumberRows;
end;
...
begin
  ...
  for I := 0 to PriGpsLocal.Count - 1 do
    GpList.Add(ConvertString(PriGpsLocal.Items[I].Name));
  case PCurrent.ActiveApp of
  aaUcode:
    begin
  ...
  begin
    ...
    rbwdgPri.Cells[1,IP1] := ConvertString(psSource.Items[I].Name);
    ...
    begin
      ...
      for I := 1 to NPri do
        begin
          if ClearedLocalSet then PriSetLocal.Add;
          II := I-1;
          PriSetLocal.Items[II].Name := ConvertString20(rbwdgPri.Cells[1,I]);
          if I>1 then
            begin
              J := 1;
              while (J<I) and (IErr=0) do
                begin
                  JJ := J-1;
                  if SameText(ConvertString(PriSetLocal.Items[II].Name),ConvertString(PriSetLocal.Items[JJ].Name)) then
                    begin
                      if not IgnoreErrors then
                        begin
                          IErr := I;
...
begin
  ...
  if PriGpsLocalChanged or PriorSetupLocalChanged then
    begin
      if not SavePriGps then OK := False;
    end;
    ...
    begin
      ...
      for I := 1 to NGps do
        begin
          if ClearedLocalGps then PriGpsLocal.Add;
          II := I-1;
          PriGpsLocal.Items[II].Name := ConvertString20(rbwdgPriGps.Cells[0,I]);
          //
          if I>1 then
            begin
              J := 1;
              while (J<I) and (IErr=0) do
                begin
                  JJ := J-1;
                  if SameText(ConvertString(PriGpsLocal.Items[II].Name), ConvertString(PriGpsLocal.Items[JJ].Name)) then
                    begin
                      IErr := I;
                      Msg := 'Error: Duplicate group name: ' + ConvertString(PriGpsLocal.Items[II].Name);
                      ShowMessage(Msg);
                    end;
                  J := J + 1;
                end;
