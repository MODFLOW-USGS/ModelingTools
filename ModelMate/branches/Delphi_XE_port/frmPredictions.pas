        ...
        begin
          ...
          if J_Valid_Name(TempString,MaxLenDepName) then
            begin
              ...
              PredSetLocal.Items[IPred].Name := TempString;
    ...
    begin
      for IGp := 0 to NGps - 1 do
        begin
          ...
          PredGpsLocal.Items[IGp].Name := TempString;
                    ...
                    begin
                      if rbwdgPredGps.RowCount > 2 then
                        ...
                        begin   // TODO 3 : I think this code is unneeded.
                          rbwdgPredGps.Cells[0,1] := '';
                          TempPredGpSet := TDepSet.Create;
                          rbwdgPredGpsPopulate(TempPredGpSet);
                          TempPredGpSet.Free;
                        end;
                      DelRow := I;
                      // Delete corresponding item in PredGpsLocal.
                      PredGpsLocal.Delete(I-1);
                    end
                  else
                    begin
...
begin
  for IRowCurr := 1 to rbwdgPredGps.RowCount - 1 do
    begin
      OldGroup := PredGpsLocal.Items[IRowCurr - 1].Name;
...
begin
  ...
  begin
    IP1 := IGP + 1;
    rbwdgPredGps.Cells[0,IP1] := dsSource.Items[IGP].Name;
    J := 0;
...
begin
  ...
  for I := 0 to PredGpsLocal.Count - 1 do
    GpList.Add(PredGpsLocal.Items[I].Name);
  case PCurrent.ActiveApp of
  aaUcode:
    begin
  ...
  begin
    ...
    rbwdgPred.Cells[1,IP1] := dsSource.Items[I].Name;
    ...
    begin
      ...
      for I := 1 to NPred do
        begin
          if ClearedLocalSet then PredSetLocal.Add;
          II := I-1;
          PredSetLocal.Items[II].Name := rbwdgPred.Cells[1,I];
          if I>1 then
            begin
              J := 1;
              while (J<I) and (IErr=0) do
                begin
                  JJ := J-1;
                  if SameText(PredSetLocal.Items[II].Name,PredSetLocal.Items[JJ].Name) then
                    begin
                      if not IgnoreErrors then
                        begin
                          IErr := I;
    ...
    begin
      ...
      for I := 1 to NGps do
        begin
          if ClearedLocalGps then PredGpsLocal.Add;
          II := I-1;
          PredGpsLocal.Items[II].Name := rbwdgPredGps.Cells[0,I];
          //
          if I>1 then
            begin
              J := 1;
              while (J<I) and (IErr=0) do
                begin
                  JJ := J-1;
                  if SameText(PredGpsLocal.Items[II].Name, PredGpsLocal.Items[JJ].Name) then
                    begin
                      IErr := I;
                      Msg := 'Error: Duplicate group name: ' + PredGpsLocal.Items[II].Name;
                      ShowMessage(Msg);
                    end;
                  J := J + 1;
                end;
