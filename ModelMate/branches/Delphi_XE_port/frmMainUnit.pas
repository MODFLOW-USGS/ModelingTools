...
uses
  ...
  frmModelCommandsUnit, frmModelIO, frmModelSettings, frmOutput, frmWait,
  ...
  IniFileUtilities, JvBaseDlg, JvProgressDialog;
...
//    procedure btnBasicUcodeClick(Sender: TObject);
  ...
  private
    ...
    procedure EnableParGroups(UseGps: boolean);
...
procedure TFormMain.EnableParGroups(UseGps: boolean);
begin
{ TODO 5 -cParameters : add code to enable/disable parameter groups table }
end;
          ...
          if GlobalBasicData.StartNewProject then
            ...
            begin
              ...
              if PCurrent.NameLikeProjectFile then
                begin
                  PCurrent.ProjName := FileBaseName(PCurrent.FileName);
                end;
...
begin
  ...
  try
    ...
    try
      ...
      if PCurrent.NameLikeProjectFile then
        PCurrent.ProjName := ChangeFileExt(ExtractFileName(PCurrent.FileName),'');
      
      StatusBar1.Panels[0].Text := ' Project Name: ' + PCurrent.ProjName;
...
begin
  ...
  begin
    ...
    StatusBar1.Panels[0].Text := ' Project Name: ' + Prj.ProjName;
    EnableSaveRevert;
  end;
  KeepCurrent := True;
end;
      ...
      if NObsGps > 0 then
        begin
          ...
          for I := 1 to NObsGps do
            begin
              GName := Project.ObsGpSet.Items[I-1].Name;
      ...
      if NPredGps > 0 then
        begin
          ...
          for I := 1 to NPredGps do
            begin
              GName := Project.PredGpSet.Items[I-1].Name;
                    ...
                    begin
                      if rbwdgParamGps.RowCount > 2 then
                        ...
                        begin   // TODO 3 : I think this code is unneeded.
                          rbwdgParamGps.Cells[0,1] := '';
                          TempParamGpSet := TParamSet.Create;
                          rbwdgParamGpsPopulate(TempParamGpSet);
                          TempParamGpSet.Free;
                        end;
                      DelRow := I;
                      // Delete corresponding item in ParamGpsCurrent.
                      ParamGpsCurrent.Delete(I-1);
                    end
                  else
                    begin
...
begin
  ...
  begin
    OldGroup := ParamGpsCurrent.Items[IRowCurr - 1].Name;
...
var
  ...
  GpName: array[1..4] of string;
  Iuobsv: array[0..4] of Integer;
  NumDeps: array[0..4] of Integer;
  NTotalDeps: Integer;
  I: Integer;
  Msg: string;
begin
  ...
  begin
    case ModelUse of
      muCalib: Msg := 'Project contains no observations.  No instruction file(s) created.';
      muPred: Msg := 'Project contains no predictions.  No instruction file(s) created.';
    end;
    ShowMessage(Msg);
  end;
end;
...
begin
  ...
  for I := 0 to ParamGpsCurrent.Count - 1 do
    GpList.Add(ParamGpsCurrent.Items[I].Name);
  ...
  begin
    IP1 := I + 1;
    rbwdgParams.Cells[0,IP1] := psSource.Items[I].Name;
    J := 0;
...
begin
  ...
  if ReturnCode = 0 then
    ...
    begin
      Errors := Errors + 1;
      JvProgressDialog1.Hide;
      Messg := 'Error: File "' + GlobalProgramLocations.Ucode2005Location + '" not found.  Please correct location of UCODE_2005 executable.';
      ShowMessage(Messg);
      OpenProgramLocations;
      ExportErrMsg := 'If location of UCODE_2005 executable has been corrected, please continue.';
    end;
  //.
  JvProgressDialog1.Hide;
  if ReturnCode = 0 then
...
//                WinExec(PAnsiChar(BatchFileLocation),SW_SHOW);
                BatchFileLocationTemp := '"' + BatchFileLocation + '"';
...
//                WinExec(PAnsiChar(BatchFileLocationTemp),SW_SHOW);
      ...
      begin
        ShowMessage('Location of UCODE_2005 executable needs to be defined.');
        OpenProgramLocations;
      end;
  finally
    slOutput.Free;
          ...
          begin
            ...
            if ParNameTemp <> '' then
              begin
                ...
                psOpt.Add;
                psOpt.Items[I-1].CreateNameValue(psOpt,ParNameTemp,
        ...
        if NPars > 0 then
          begin
            ...
            begin
              ...
              psPval.Add;
              psPval.Items[I].CreateNameValue(psPval, PName, aGroup, PVal);
              IndexPF := IndexPF + 1;
            end;
          end;
      end;
...
begin
  ...
  begin
    IP1 := IGP + 1;
    rbwdgParamGps.Cells[0,IP1] := psSource.Items[IGP].Name;
    J := 0;
    for IA := 0 to NumParAttributes - 1 do
    begin
...
begin
  ...
  if sdProject.Execute then
    begin
      ...
      if (FileExists(SdProject.FileName)) and (SdProject.FileName <> PCurrent.FileName) then
        begin
          Messg := SdProject.FileName + ' already exists.  Do you want to replace it?';
          MR := MessageDlg(Messg,mtConfirmation,[mbYes,mbNo],0);
          OK := MR = mrYes;
        end;
      if OK then
        begin
          ProjectDirectory := ExtractFileDir(SdProject.FileName);
          PCurrent.FileName := sdProject.FileName;
          if PCurrent.NameLikeProjectFile then
            begin
              PCurrent.ProjName := FileBaseName(PCurrent.FileName);
            end;
      ...
      if AssignParameters then
        begin
          ...
          if TempDir <> '' then
            ...
            begin
              TempDir := RelDirToAbsDir(ProjectDirectory, '\');
...
var
  ...
  AbsModelDir, OrigDir, Status0Text: string;
...
var
  ...
  TempValue, TimeoutFac, Wait: double;
  OK: boolean;
...
begin
  OK := true;
  TempCommand := StringOfChar(' ',255);
  TempPurpose := StringOfChar(' ',255);
  UcFile := StringOfChar(' ',255);
  CMaxChgRealm := StringOfChar(' ',255);
  CTrustReg := StringOfChar(' ',255);
  Keyword := StringOfChar(' ',255);
  ParamGpName := StringOfChar(' ',255);
  ...
  if odUcode.Execute then
    begin
      ...
      UcodeFile := odUcode.FileName;
      UcFile := UcodeFile;
      Str := FileBaseName(UcodeFile);
      PCurrent.ProjName := Str;
      ProjFileTemp := Str + '.mtc';
      Invalidate;
      // Set up progress bar in status bar.
...
//      StatusBar1.Panels[2].Width := 5000;
//      ProgressBar1.Position := 0;
//      ProgressBar1.Left := StatusBar1.Panels[0].Width + 20;
//      ProgressBar1.Top := 3;
//      ProgressBar1.Width := StatusBar1.Width - StatusBar1.Panels[0].Width - 30;
...
//      ProgressBar1.Visible := True;
  ...
  Screen.Cursor := crHourglass;
  LenStr1 := 255;
  Deriv_Interface := StringOfChar(' ',255);
  try
    if OK then
      begin
      ...
      begin
        ...
        if IFail = 0 then
          begin
            ...
            PCurrent.UcProject.DerivativesInterface := Deriv_Interface;
            PCurrent.UcProject.PathToMergedFile := Path2MergedFile;
      ...
      begin
        ...
        if IFail = 0 then
          begin
            ...
            PCurrent.UcProject.StartSens := StrtSens;
            PCurrent.UcProject.IntermedSens := IntSens;
            PCurrent.UcProject.FinalSens := FinSens;
      ...
      begin
        ...
        if NCommands > 0 then
          begin
            ...
            PCurrent.MCLForward := TempCommand;
            TempPurpose := 'Forward&Der';
            aj_get_model_command_by_purpose(TempPurpose, TempCommand, LenStr1, LenStr1);
            PCurrent.MCLFwdDeriv := TempCommand;
          end;
        JvProgressDialog1.Position := 10;
          ...
          begin
            ...
            for I := 0 to NParGps - 1 do
              begin
                ...
                TempStr := FloatToStr(TempValue);
                ParamGpsCurrent.Items[I].SetAttributeByType(patLowerValue,TempStr);
                ...
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                TempStr := FloatToStr(TempValue);
                ParamGpsCurrent.Items[I].SetAttributeByType(patUpperValue,TempStr);
                if TempStr <> '1E39' then
                  begin
                    ReasonableRangeControlMethod := cmByGroup;
                  end;
          ...
          begin
            ...
            for I := 0 to NPars - 1 do
              begin
                ...
                ParamSetCurrent.Items[I].SetAttributeByType(patParamName,TempStr);
                Keyword := 'PARGROUPNAME';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ParamSetCurrent.Items[I].SetAttributeByType(patGroupName,TempStr);
                // Doubles.
                Keyword := 'STARTVALUE';
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                ...
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                TempStr := FloatToStr(TempValue);
                ParamSetCurrent.Items[I].SetAttributeByType(patLowerValue,TempStr);
                if TempStr <> '-1E39' then
                  begin
                ...
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                TempStr := FloatToStr(TempValue);
                ParamSetCurrent.Items[I].SetAttributeByType(patUpperValue,TempStr);
                if TempStr <> '1E39' then
                  begin
                    ReasonableRangeControlMethod := cmByItem;
                  end;
          ...
          begin
            for I := 0 to NDerPars - 1 do
              begin
                ...
                ParamSetCurrent.Items[J].SetAttributeByType(patParamName,TempParName);
                Keyword := 'DERPAREQN';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ParamSetCurrent.Items[J].DerivEqn := TempStr;
          ...
          begin
            ...
            for I := 0 to NObsGps - 1 do
              begin
                ...
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ObsGpsCurrent.Items[I].SetAttributeByType(datGroupName,TempStr);
                Keyword := 'GPSTATFLAG';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ObsGpsCurrent.Items[I].SetAttributeByType(datStatFlag,TempStr);
                Keyword := 'COVMATRIX';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ObsGpsCurrent.Items[I].SetAttributeByType(datCovMatrix,TempStr);
                // Doubles.
          ...
          begin
            ...
            for I := 0 to NObs - 1 do
              begin
                ...
                Keyword := 'OBSNAME';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ObsSetCurrent.Items[I].SetAttributeByType(datObsName,TempStr);
                Keyword := 'STATFLAG';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ObsSetCurrent.Items[I].SetAttributeByType(datStatFlag,TempStr);
                ...
                Keyword := 'GROUPNOBS';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ObsSetCurrent.Items[I].SetAttributeByType(datGroupName,TempStr);
                Keyword := 'DEROBSEQN';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                ObsSetCurrent.Items[I].SetAttributeByType(datEquation,TempStr);
                // Doubles.
                Keyword := 'OBSVALUE';
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                ...
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                TempStr := FloatToStr(TempValue);
                ObsSetCurrent.Items[I].SetAttributeByType(datStatistic,TempStr);
                if TempStr <> '1E39' then
                  begin
          ...
          begin
            ...
            for I := 0 to NPredGps - 1 do
              begin
                ...
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PredGpsCurrent.Items[I].SetAttributeByType(datGroupName,TempStr);
                Keyword := 'GPMEASSTATFLAG';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PredGpsCurrent.Items[I].SetAttributeByType(datMeasStatFlag,TempStr);
                // Doubles.
          ...
          begin
            ...
            for I := 0 to NPreds - 1 do
              begin
                ...
                Keyword := 'PREDNAME';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PredSetCurrent.Items[I].SetAttributeByType(datPredName,TempStr);
                Keyword := 'MEASSTATFLAG';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PredSetCurrent.Items[I].SetAttributeByType(datMeasStatFlag,TempStr);
                if TempStr = 'UNKNOWN' then
                  begin
                ...
                Keyword := 'GROUPNPRED';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PredSetCurrent.Items[I].SetAttributeByType(datGroupName,TempStr);
                Keyword := 'DERPREDEQN';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PredSetCurrent.Items[I].SetAttributeByType(datEquation,TempStr);
                // Doubles.
                Keyword := 'REFVALUE';
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                ...
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                TempStr := FloatToStr(TempValue);
                PredSetCurrent.Items[I].SetAttributeByType(datMeasStatistic,TempStr);
                if TempStr <> '1E39' then
                  begin
                    StatisticControlMethod := cmByItem;
                  end;
          ...
          begin
            ...
            for I := 0 to NPriGps - 1 do
              begin
                ...
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriGpsCurrent.Items[I].SetAttributeByType(piatGroupName,TempStr);
                Keyword := 'GPPRISTATFLAG';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriGpsCurrent.Items[I].SetAttributeByType(piatStatFlag,TempStr);
                Keyword := 'PRICOVMATRIX';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriGpsCurrent.Items[I].SetAttributeByType(piatCovMatrix,TempStr);
                Keyword := 'GPPRIEQN';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriGpsCurrent.Items[I].SetAttributeByType(piatEquation,TempStr);
                // Doubles.
          ...
          begin
            ...
            for I := 0 to NPri - 1 do
              begin
                ...
                Keyword := 'PRIORNAME';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriSetCurrent.Items[I].SetAttributeByType(piatPriorName,TempStr);
                Keyword := 'PRISTATFLAG';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriSetCurrent.Items[I].SetAttributeByType(piatStatFlag,TempStr);
                if TempStr = 'UNKNOWN' then
                  begin
                ...
                Keyword := 'PRIEQN';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriSetCurrent.Items[I].SetAttributeByType(piatEquation,TempStr);
                Keyword := 'GROUPNPRI';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PriSetCurrent.Items[I].SetAttributeByType(piatGroupName,TempStr);
                // Doubles.
                ...
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
                TempStr := FloatToStr(TempValue);
                PriSetCurrent.Items[I].SetAttributeByType(piatStatistic,TempStr);
                if TempStr <> '1E39' then
                  begin
                    StatisticControlMethod := cmByItem;
                  end;
          ...
          begin
            ...
            for I := 0 to NMIFiles - 1 do
              begin
                ...
                Keyword := 'MODINFILE';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PCurrent.MIFiles.Items[I].ModelFile := TempStr;
                Keyword := 'TEMPLATEFILE';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PCurrent.MIFiles.Items[I].AppFile := TempStr;
              end;
          end;
          ...
          begin
            ...
            for I := 0 to NMOFiles - 1 do
              begin
                ...
                Keyword := 'MODOUTFILE';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PCurrent.MOFiles.Items[I].ModelFile := TempStr;
                Keyword := 'INSTRUCTIONFILE';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PCurrent.MOFiles.Items[I].AppFile := TempStr;
              end;
          end;
...
//        ProgressBar1.Position := 98;
    ...
    if OK then
      begin
        self.Invalidate;
        aj_ini_parallel(KCtrl, NumRunners, IParallel, Wait, VerboseRunner,
                        IAutoStopRunners, TimeoutFac);
        if KCtrl > 0 then
          begin
          ...
          begin
            ...
            for I := 0 to NumRunners - 1 do
              begin
                ...
                Keyword := 'RUNNERNAME';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PCurrent.UcProject.ParallelRunners.Items[I].Name := TempStr;
                Keyword := 'RUNNERDIR';
                aj_get_character_by_keyword(Keyword, IVar, TempStr, LenStr1, LenStr1);
                PCurrent.UcProject.ParallelRunners.Items[I].Directory := TempStr;
                Keyword := 'RUNTIME';
                aj_get_double_by_keyword(Keyword, IVar, TempValue, LenStr1);
            ...
            if psTemp.Count = ParamSetCurrent.Count then
              begin
                ...
                while (I < psTemp.Count) and OK do
                  begin
                    OK := AnsiSameText(psTemp.Items[I].Name,
                                ParamSetCurrent.Items[I].Name);
                    I := I + 1;
                  end;
              end
            else
    ...
    begin
      for IParGp := 0 to NParGps - 1 do
        begin
          ...
          ParamGpsCurrent.Items[IParGp].Name := TempString;
                            ...
                            if rbwdgParams.Cells[0,IPar+1] <> '' then
                              begin
                                ...
                                while (I < PSetTemp.Count) and (not FoundMatch) do
                                  begin
                                    if AnsiSameText(PsetTemp.Items[I].Name,TempName) then
                                      begin
...
begin
  ...
  begin
    ...
    begin
      if ParamGpsCurrent.Items[J].Name = GpName then
        IPG := J;
    end;
    // Assign attributes to this parameter group, as defined in table
    // Loop through columns (parameter attributes)
    ...
    begin
      ...
      begin
        ...
        ReplaceParAttValue(TempString, PatTemp);
        ParamGpsCurrent.Items[IPG].AllAtts[IPA].Text := TempString;
        ParamGpsCurrent.Items[IPG].Name := NewGroup;
      end;
    end;
  end;
  AssignParameterGroups;
