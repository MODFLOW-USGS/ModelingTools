          ...
          begin
            ...
            if KIndex > -1  then
              begin
                ...
                case ModelUse of
                  muCalib: slInsFiles[KIndex].Add(aProject.ObsSet.Items[J].Name);
                  muPred: slInsFiles[KIndex].Add(aProject.PredSet.Items[J].Name);
                end;
                KI := J;
              end
            else
    ...
    begin
      ...
      if GetNameFileEntry(NameFile, 'PVAL', UnitNum, PvalFile) then
        begin
          ...
          PvalFile := aProject.AbsModelDirectory(muCalib) + '\' + PvalFile;
        end
      else
        begin
          ...
          try
            ...
            PvalFile := aProject.AbsModelDirectory(ModelUse) + '\' + aProject.ProjName + '.pval';
  ...
  if OK then
    begin
      ...
      if IsNonBlank(PValTplFile) then
        begin
          PValTplFile := AbsAppDir + '\' + PValTplFile;
        end
      else
        begin
          PValTplFile := AbsAppDir + '\' + NameFileBase + '_pval.' + Ext;
        end;
      ...
      try
        ...
        Line := '# MODFLOW-2005 PVAL file for project ' + aProject.ProjName;
        slPValTplFile.Add(Line);
        ...
        for I := 0 to aProject.ParamSet.Count - 1 do
          begin
            Par := aProject.ParamSet.Items[I].Name;
            ...
            if OK then
              begin
                ...
                and StatFlag for Flow observations? }
                          dsTemp.Items[dsTemp.Count - 1].DefineDep(OBSNAM, GroupName,
...
and StatFlag for Head and Head-change observations? }
              dsTemp.Items[dsTemp.Count - 1].DefineDep(OBSNAM, GroupObs,
                                                       HOBS, PSobs, dcObs);
              NumObs := NumObs + 1;
            end;
        end
...
and StatFlag for Head observations? }
          dsTemp.Items[dsTemp.Count - 1].DefineDep(OBSNAM, GpHeads,
...
and StatFlag for Head and Head-change observations? }
              dsTemp.Items[dsTemp.Count - 1].DefineDep(OBSNAM, GroupDep,
                                                       HOBS, PSdep, aDepCat);
              NumDep := NumDep + 1;
            end;
        end
...
//procedure TProgressOwner.Create(AOwner: TComponent);
//begin
//  inherited Create(AOwner);
//  JvProgressDialog1 := TJvProgressDialog.Create(self);
//  JvProgressComponent1 := TJvProgressComponent.Create(self);
//end;
//
//procedure TProgressOwner.Destroy;
//begin
//  JvProgressDialog1.Free;
//  JvProgressComponent1.Free;
//  inherited;
//end;

initialization
//  ProgressOwner1 := TProgressOwner
