        ...
        begin
          ...
          if J_Valid_Name(TempString,MaxLenDepName) then
            begin
              ...
              ObsSetLocal.Items[IObs].Name := TempString;
    ...
    begin
      for IObsGp := 0 to NObsGps - 1 do
        begin
          ...
          ObsGpsLocal.Items[IObsGp].Name := TempString;
...
begin
  for IRowCurr := 1 to rbwdgObsGps.RowCount - 1 do
    begin
      OldGroup := ObsGpsLocal.Items[IRowCurr - 1].Name;
                    ...
                    begin
                      if rbwdgObsGps.RowCount > 2 then
                        ...
                        begin   // TODO 3 : I think this code is unneeded.
                          rbwdgObsGps.Cells[0,1] := '';
                          TempObsGpSet := TDepSet.Create;
                          rbwdgObsGpsPopulate(TempObsGpSet);
                          TempObsGpSet.Free;
                        end;
                      DelRow := I;
                      // Delete corresponding item in ObsGpsLocal.
                      ObsGpsLocal.Delete(I-1);
                    end
                  else
                    begin
...
begin
  ...
  begin
    IP1 := IGP + 1;
    rbwdgObsGps.Cells[0,IP1] := dsSource.Items[IGP].Name;
    J := 0;
    ...
    if NeedObsPopulate then
      begin
        for I := 0 to ObsGpsLocal.Count - 1 do
          GpList.Add(ObsGpsLocal.Items[I].Name);
        case PCurrent.ActiveApp of
        aaUcode:
          begin
        ...
        for I := 0 to dsSource.Count - 1 do
          begin
            ...
            rbwdgObs.Cells[1,IP1] := dsSource.Items[I].Name;
...
//          UpdateCurrentProject;
        ...
        begin
          ...
          rbwdgObs.Cells[1, IRowCurr] := ObsSetLocalTemp.Items[IRowCurr-1].Name;
          IgnoreChange := True;
        end;
    end;
    ...
    begin
      ...
      for I := 1 to NObs do
        begin
          if ClearedLocalSet then ObsSetLocal.Add;
          II := I-1;
          ObsSetLocal.Items[II].Name := rbwdgObs.Cells[1,I];
          if I>1 then
            begin
              J := 1;
              while (J<I) and (IErr=0) do
                begin
                  JJ := J-1;
                  if SameText(ObsSetLocal.Items[II].Name,ObsSetLocal.Items[JJ].Name) then
                    begin
                      if not IgnoreErrors then
                        begin
                          IErr := I;
    ...
    begin
      ...
      for I := 1 to NObsGps do
        begin
          if ClearedLocalGps then ObsGpsLocal.Add;
          II := I-1;
          ObsGpsLocal.Items[II].Name := rbwdgObsGps.Cells[0,I];
          //
          if I>1 then
            begin
              J := 1;
              while (J<I) and (IErr=0) do
                begin
                  JJ := J-1;
                  if SameText(ObsGpsLocal.Items[II].Name, ObsGpsLocal.Items[JJ].Name) then
                    begin
                      IErr := I;
                      Msg := 'Error: Duplicate group name: ' + ObsGpsLocal.Items[II].Name;
                      ShowMessage(Msg);
                    end;
                  J := J + 1;
                end;
