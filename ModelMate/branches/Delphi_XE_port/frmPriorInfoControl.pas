...
type
  ...
  private
    ...
    procedure EnablePrior(B: Boolean);
...
procedure TFormPriorInfoControl.EnablePrior(B: Boolean);
begin
  PCurrent.UsePriGps := B;
  if B then
    begin
      cbUsePriorInfo.Checked := True;
      lblNumPri.Font.Color := clWindowText;
      lblNumPriGps.Font.Color := clWindowText;
      dgPriGpSummary.Enabled := True;
      dgPriGpSummary.Font.Color := clWindowText;
      dgPriGpSummary.Columns[0].Font.color := clWindowText;
      dgPriGpSummary.Columns[1].Font.color := clWindowText;
      btnPriorForm.Enabled := True;
    end
  else
    begin
      cbUsePriorInfo.Checked := False;
      lblNumPri.Font.Color := clGrayText;
      lblNumPriGps.Font.Color := clGrayText;
      dgPriGpSummary.Enabled := False;
      dgPriGpSummary.Font.Color := clGrayText;
      dgPriGpSummary.Columns[0].Font.color := clGrayText;
      dgPriGpSummary.Columns[1].Font.color := clGrayText;
      btnPriorForm.Enabled := False;
    end;
end;
  ...
  if NPriGps > 0 then
    begin
      ...
      for I := 1 to NPriGps do
        begin
          GName := PriGpsControl.Items[I-1].Name;
