...
type
  TfrmMain = class(TForm)
    ...
    tvModflow: TTreeView;
    SelectMODFLOW_ListingFiles: TMenuItem;
    btnDelete: TButton;
    ...
    procedure SelectMODFLOW_ListingFilesClick(Sender: TObject);
...
begin
  ...
  try
    ...
    begin
      ...
      ListingFileName := ParentNode.Text;
      ModelDirectory := ExtractFileDir(ListingFileName);
      ...
      StatusBar1.SimpleText := 'testing ' + ListingFileName;
      ActivApp1.ExePath := Format('"%0:s" "%1:s" -e', [ProgramExeName, ListingFileName]);
      ActivApp1.ExecuteApp(Success);
      pbFiles.StepIt;
      if not Success then
      begin
        result := false;
        ErrorMessage := 'Failed to execute "' + ListingFileName + '".';
        Exit;
      end;
...
begin
  ...
  begin
    Beep;
    MessageDlg('ListingAnalyst location has not been specified.', mtError, [mbOK], 0);
    Exit;
  end;
...
begin
  ...
  if (not ModelNode.HasChildren) and (ModelNode.Parent <> nil) then
  begin
    ModelNode := ModelNode.Parent;
  end;
  if ModelNode.HasChildren then
  begin
...
procedure TfrmMain.SelectMODFLOW_ListingFilesClick(Sender: TObject);
var
  Index: integer;
