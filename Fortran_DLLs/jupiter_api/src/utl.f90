...
!        '-- STOP EXECUTION (UTL_FILTERLIST_C)')
  ...
  SUBROUTINE UTL_GETTOKEN(KERR,INUNIT,IOUT,NCODE,ICOL,LINE,ISTART,ISTOP,TOKEN)
    ...
    USE GLOBAL_DATA, ONLY: BLANKS, MAX_STRING_LEN, AMESSAGE ! Added for ModelMate
    IMPLICIT NONE
    !
    !   Argument-list variables
    ...
    IF (MKSQ>0 .OR. MKDQ>0) THEN
      ...
      ELSE
        ...
        IF (AMESSAGE == ' ') AMESSAGE = 'ERROR: Unmatched quotes in line: ' // TRIM(LINE) ! Added for ModelMate
        KERR = KERR+1
      ENDIF
    ENDIF
...
!    30 FORMAT(I10,F10.0,A20,I10)
  ...
  SUBROUTINE UTL_READLIST(BLOCKERR,BLOCKLABEL,ENDKEYS,INUNIT,IOUT,KEYEQUAL,  &
    ...
    USE DATATYPES
    USE GLOBAL_DATA, ONLY: BLANKS, MAX_STRING_LEN, AMESSAGE
    IMPLICIT NONE
    !
    !   Argument-list variables
    ...
    IF (IERR > 0) THEN
      WRITE(IOUT,150) IERR
      IF (AMESSAGE == ' ') AMESSAGE = ' Errors found (UTL_READLIST)'
      CALL UTL_STOP(' ')
    ENDIF
...
!          WRITE(*,10) STOPMESS
        ...
        ! Add for ModelMate
        IF (AMESSAGE .NE. ' ') THEN
          AMESSAGE = 'Unidentified error; output file may contain an error message'
        ELSE
          IF (INDEX(AMESSAGE,STOPMESS) == 0) AMESSAGE = TRIM(AMESSAGE) // ' ' // TRIM(STOPMESS)
        ENDIF
      ENDIF
    ENDIF
    !STOP  ! Edit for ModelMate
    GERR = .TRUE.
    IF (AMESSAGE == ' ') THEN
      AMESSAGE = 'Unidentified error; output file may contain an error message'
    ENDIF
    ERRMESSAGE = TRIM(AMESSAGE)
    RETURN
...
100 CONTINUE
    ...
    !
200 CONTINUE
    !CALL UTL_STOP()  ! Comment out for ModelMate, maybe for API since UTL_STOP calls UTL_WRITE_MESSAGE
    !
