MODULE DEPENDENTS
  ...
  PUBLIC :: GROUP, GROUPNAM, GROUPNOBS, IGPTYPE, LLPTRDEP, LLPTRDEPGP,   &
            ...
            OBSVAL, USE_FLAGDEP, USEFLAG, WTCORRELATED, WTCOS, &
  !   Make selected variables PUBLIC for linking with ModelMate
            PLOTSYMBOL, NOBSGPS, WTMULTIPLIER, COVMATNAM, NOBSCOLS, OBSCOL, &
            OBSALLNAM
  !
...
CONTAINS
  ...
  SUBROUTINE DEP_INI_FILTERLIST(DEFGROUP,IOUT,NOBS,NEQNPTR,OBSNAM)
    ...
    NULLIFY(PTR,PIPTR)
    !   Allocate and initialize arrays related to observations
    ALLOCATE(NONDETVAL(NOBS),OBSVAL(NOBS), &
  ...
  SUBROUTINE DEP_INI_READ(IDEPTYPE,INUNIT,IOUT,NCOVMAT,NOBS,NDDEP,NEDEP,NTDEP)
    ...
    CALL UTL_READBLOCK(0,TRIM(GPSBLOCK(IDEPTYPEIN)),(/' '/),INUNIT, &
  ...
  SUBROUTINE DEP_INI_STORE(IDEPTYPE,IOUT,NCOVMAT,NOBS,COVMATARR,NEQNPTR, &
    ...
    IF (IFAIL > 0) THEN
      AMESSAGE = &
      'Failed in DEP_INI_STORE, cannot take log of zero or negative number'
      CALL UTL_WRITE_MESSAGE(IOUT,'yes','yes','yes')
      CALL UTL_STOP(' ')
    ENDIF
    !
