...
CONTAINS
  ...
  SUBROUTINE PRI_INI_POPX(IOUT,IPRC,MPR,NPE,NPT,IPTR,LN,PARNAM,XLOG,XPRI, &
    ...
    IF (MPR>0 .AND. .NOT. XPRIFILLED) THEN
      ...
      DO I=1,MPR
        CHECKLOG: DO J=1,NPE
          IF(XPRI(J,I) .NE. 0 .AND. LN(IPTR(J)).EQ.1) THEN
            ...
            ! TRANSFORMED WEIGHTS ARE IN LOG10 SPACE, TRANSFORM TO LN SPACE
            DO II=1,MPR
      ...
      WRITE(IOUT,*)' (TRANSFROMED PARAMETERS ARE REPORTED IN NATURAL LOG SPACE)'
      CALL PRI_INI_WRITEPRIS(IOUT,MPR)
      CALL UTL_CHECK_NAMES(IOUT,MPR,PRINAM)
      IF (IVERB>2) THEN
        WRITE(IOUT,*)' '
        WRITE(IOUT,*)' '
        WRITE(IOUT,*)'PRIOR INFORMATION WEIGHTS NATURAL LOG SPACE:'
  ...
  SUBROUTINE PRI_INI_PROCESS(IOUT,IWRITE,MPR,NCOVMAT,COVMATARR,DTLA)
    ...
    !
    IF (IFAIL > 0) THEN
      AMESSAGE = 'Failed in PRI_INI_PROCESS, cannot take log(-x)'
      CALL UTL_WRITE_MESSAGE(IOUT,'yes','yes','yes')
      CALL UTL_STOP(' ')
    ENDIF
    CALL UTL_CHECK_NAMES(IOUT,MPR,PRINAM)
    ...
    !WTDPRI = UTL_MATMULVEC(MPR,PRIWTMATSQR,PRIVAL)
  ...
  SUBROUTINE PRI_INI_READ(INUNIT,IOUT,IWRITE,NPE,NCOVMAT,MPR)
    ...
    CALL UTL_READBLOCK(0,'PRIOR_INFORMATION_GROUPS',(/' '/),INUNIT,   &
  ...
  SUBROUTINE PRI_INI_STORE_WPRED(IOUT,MPR,NPE,NPT,ITRANS,PARNAMLC,PVAL, &
                           XPRIFILLED,CONVERT_STAT,PRED,MPRWOPPAS)
    !   Store data read from prior-information input blocks
    USE GLOBAL_DATA, ONLY: BIGDOUBLE, IVERB, MAX_STRING_LEN
    USE UTILITIES
    USE EQUATION, ONLY: EQN_LINEAR_COEFFS
