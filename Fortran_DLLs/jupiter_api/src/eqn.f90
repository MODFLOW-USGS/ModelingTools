...
!    If CONVERT_STAT is true this routine determines whether the equation is
!    for a single parameter that is log transformed. If so and if it the
!    parameter does not appear as log10(parametername) then the equation will
!    be editted to be of that form. If it is already of that form, the code
!    will proceed as usual.  If such a parameter occurs in any other form,
!    an error message will be written and the code will terminate.
